...
pub fn prove(setup: &Setup, entries: &[Fr], index: usize, value: Fr) -> Proof {
    ...
    // verifier provides s1 and s2 challenges for Proof of Opening of a Pedersen Commitment
    let s1 = challenge_scalar(&mut transcript, b"ped_opening_challenge_1");
    append(&mut transcript, b"ped_opening_challenge_1", s1);
    let s2 = challenge_scalar(&mut transcript, b"ped_opening_challenge_2");
    append(&mut transcript, b"ped_opening_challenge_2", s2);
...
pub fn verify(setup: &Setup, entries: &[Fr], proof: Proof) -> bool {
    ...
    // Verifier can compute R themselves
    let R = pedersen_commit(s1, s2, g1, h);

    // pedersen opening proof verification
    append(&mut transcript, b"commitment_to_opening_challenges", R);
