...
pub struct Setup {
    ...
    // We need another multiplicative subgroup!
...
pub fn prove(setup: &Setup, entries: &[Fr], index: usize, value: Fr) -> Proof {
    ...
    // Prover provides random s1 and s2 challenges for ZK Proof of Opening of a Pedersen Commitment
    let s1 = Fr::rand(rng);
    append(&mut transcript, b"ped_opening_blinding_1", s1);
    let s2 = Fr::rand(rng);
    append(&mut transcript, b"ped_opening_blinding_2", s2);

    // proof for Pederson opening
    let R = pedersen_commit(s1, s2, g1, h);
    append(&mut transcript, b"commitment_to_blinding", R);
    ...
    // sample random values for proof of unity
    let mut r_vec = Vec::new();
    r_0 = Fr::rand(rng);
    r_vec.push(r_0);
    append(&mut transcript, b"randomness_unitiy_proof_0", r_0);
    r_1 = Fr::rand(rng);
    r_vec.push(r_1);
    append(&mut transcript, b"randomness_unitiy_proof_1", r_1);
    r_2 = Fr::rand(rng);
    r_vec.push(r_2);
    append(&mut transcript,b"randomness_unitiy_proof_2", r_2);
    r_3 = Fr::rand(rng);
    r_vec.push(r_3);
    append(&mut transcript, b"randomness_unitiy_proof_3", r_3);
    let r_poly = DensePolynomial::from_coefficients_vec(r_vec);
